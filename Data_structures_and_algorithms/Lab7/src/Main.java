		...
		while(!halt) {
			...
			if(word[0].equalsIgnoreCase("get") && word.length==2) {
				if(currentDoc!=null) {
					...
					try {
						Link l=currentDoc.links.get(index);
			...
			if(word[0].equalsIgnoreCase("index") && word.length==2) {
				if(currentDoc!=null) {
					int index=currentDoc.links.indexOf(new Link(word[1]));
			...
			if(word[0].equalsIgnoreCase("remi") && word.length==2) {
				if(currentDoc!=null) {
					int index=Integer.parseInt(word[1]);
					try {
						Link l=currentDoc.links.remove(index);
			...
			if(word[0].equalsIgnoreCase("rem") && word.length==2) {
				if(currentDoc!=null) {
					System.out.println(currentDoc.links.remove(new Link(word[1])));
				}
				else
					System.out.println("no current document");					
				continue;
			...
			if(word[0].equalsIgnoreCase("remall") && word.length==2) {
				if(currentDoc!=null) {
					currentDoc.links.removeAll(new Link(word[1]));
				}
				else
					System.out.println("no current document");	
				continue;
